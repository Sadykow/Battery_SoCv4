...
class RobustAdam(tf.keras.optimizers.Optimizer):
  ...
  def _resource_apply_dense(self, grad, var, apply_state=None):
    ...
    m_t = state_ops.assign(m, coefficients['beta_1_t'] * m,
    ...
    v_t = state_ops.assign(v, coefficients['beta_2_t'] * v,
                           use_locking=self._use_locking)
    with ops.control_dependencies([v_t]):
      v_t = state_ops.assign_add(ref=v, value=v_scaled_g_values,
            use_locking=self._use_locking, name=None)

    # d_t = beta3 * d + (1 - beta3) * relat
    d = self.get_slot(var, 'd')
    #print(tf.keras.backend.eval(var))
    # print(var)
    
    prev_var = self.get_slot(var=var, slot_name='prev_var_0')
    if self._is_first:
      # W_t = W - lr * m_t / (sqrt(v_t) + epsilon)
      var_update = state_ops.assign_sub(
          var, coefficients['lr'] * m_t / (math_ops.sqrt(v_t) + coefficients['epsilon']),
          use_locking=self._use_locking)
      prev_var.assign(var)
      return control_flow_ops.group(*[var_update, m_t, v_t])
    else:
      r_t = math_ops.abs(self.loss(var, prev_var))
      d_scaled_r_values = coefficients['one_minus_beta_3_t'] * r_t
    
      d_t = state_ops.assign(d, coefficients['beta_3_t'] * d,
                            use_locking=self._use_locking)
      with ops.control_dependencies([d_t]):
        d_t = state_ops.assign_add(ref=d, value=d_scaled_r_values,
              use_locking=self._use_locking, name=None)
      prev_var.assign(var)

      # W_t = W - lr * m_t / (d_t * sqrt(v_t) + epsilon)
      var_update = state_ops.assign_sub(
          var, coefficients['lr'] * m_t /\
              (d_t * math_ops.sqrt(v_t) + coefficients['epsilon']),
          use_locking=self._use_locking)
